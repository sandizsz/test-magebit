...
const setup = () => {
  const input = screen.queryByRole('input') || screen.queryByRole('textbox');
...
test('has colombia email validation', () => {
  ...
  expect(screen.getByText('We are not accepting subscriptions from Colombia emails')).toBeInTheDocument();
});
...
test('has error message clearing', () => {
  ...
  expect(() => screen.getByText('We are not accepting subscriptions from Colombia emails')).toThrow();
});